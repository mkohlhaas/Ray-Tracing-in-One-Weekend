...
static int *
perlin_generate_perm (perlin_phase_t *p)
{
...
perlin_phase_new ()
{
  perlin_phase_t *p = malloc (sizeof *p);
  if (!p)
    {
      logExit ("Out of memory");
    }
...
void
perlin_phase_del (perlin_phase_t *p)
{
  free (p->randvec);
  free (p->perm_x);
  free (p->perm_y);
...
static double
perlin_phase_noise_internal (perlin_phase_t *p, const point3_t *point)
{
...
double
perlin_phase_noise (perlin_phase_t *p, point3_t const *point, int depth)
{
  auto     accum  = 0.0;
  ...
  for (int i = 0; i < depth; i++)
    {
      accum += weight * perlin_phase_noise_internal (p, &temp_p);
      weight *= 0.5;
