...
static int *
perlin_generate_perm (perlin_hermitian_t *p)
{
...
perlin_hermitian_t *
perlin_hermitian_new ()
{
  perlin_hermitian_t *p = malloc (sizeof *p);
  if (!p)
    {
      logExit ("Out of memory");
    }
...
void
perlin_hermitian_del (perlin_hermitian_t *p)
{
  free (p->randfloat);
  free (p->perm_x);
  free (p->perm_y);
...
double
perlin_hermitian_noise (perlin_hermitian_t *p, const point3_t *point)
{
