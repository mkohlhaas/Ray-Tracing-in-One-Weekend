...
static int *
perlin_generate_perm (perlin_lattice_t *p)
{
...
perlin_lattice_new ()
{
  perlin_lattice_t *p = malloc (sizeof *p);
  if (!p)
    {
      logExit ("Out of memory");
    }
...
void
perlin_lattice_del (perlin_lattice_t *p)
{
  free (p->randvec);
  free (p->perm_x);
  free (p->perm_y);
...
double
perlin_lattice_noise (perlin_lattice_t *p, const point3_t *point)
{
