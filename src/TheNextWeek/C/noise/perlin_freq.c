...
static int *
perlin_generate_perm (perlin_freq_t *p)
{
...
perlin_freq_new ()
{
  perlin_freq_t *p = malloc (sizeof *p);
  if (!p)
    {
      logExit ("Out of memory");
    }
...
void
perlin_freq_del (perlin_freq_t *p)
{
  free (p->randfloat);
  free (p->perm_x);
  free (p->perm_y);
...
double
perlin_freq_noise (perlin_freq_t *p, const point3_t *point)
{
