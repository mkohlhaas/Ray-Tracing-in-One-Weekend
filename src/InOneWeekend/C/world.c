...
world_init (void)
{
  ...
  sphere       *s1 = sphere_new ((point3){ .x = 0.0, .y = -1000.0, .z = 0.0 }, 1000.0, (material_t *)material_ground);
  arrput (world, (hit_able_t *)s1);

  dielectric *material1 = dielectric_new (1.50);
  sphere     *s2        = sphere_new ((point3){ .x = 0, .y = 1, .z = 0 }, 1.0, (material_t *)material1);
  arrput (world, (hit_able_t *)s2);

  lambertian_t *material2 = lambertian_new ((color_t){ .r = 0.4, .g = 0.2, .b = 0.1 });
  sphere       *s3        = sphere_new ((point3){ .x = -4, .y = 1, .z = 0 }, 1.0, (material_t *)material2);
  arrput (world, (hit_able_t *)s3);

  metal  *material3 = metal_new ((color_t){ .r = 0.7, .g = 0.6, .b = 0.5 }, 0.0);
  sphere *s4        = sphere_new ((point3){ .x = 4, .y = 1, .z = 0 }, 1.0, (material_t *)material3);
              ...
              if (choose_mat < 0.8)
                {
                  ...
                  sphere       *s      = sphere_new (center, 0.2, (material_t *)m);
